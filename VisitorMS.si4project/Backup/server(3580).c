...
/* 公聊 */
static void broadcast_server(int fd, char *name, struct protocol *msg, int keygen)
{
	int i;
	char buf[128] = {0};
...
/* 私聊 */
static void private_server(int fd, char *name, struct protocol *msg, int keygen)
{
	int dest_index;
	char buf[128] = {0};
...
void login(int sockfd, int *index, struct protocol *msg, int keygen)
{
	...
	// check name && passwd
	// 下面的程序好像是判断用户是否在线的，这里好像没有判断密码是否正确的???待确定
...
void *func(void *arg)
{
	...
	while (1)
	{
		...
		case LOGIN:
			...
			break;
		case BROADCAST:
			broadcast_server(sockfd, name, &msg, keygen);
			break;
		case PRIVATE:
			private_server(sockfd, name, &msg, keygen);
			break;
		case ONLINEUSER:
