...
static bool postUrl(int numOfIdentity)
{
	...
	printf("--------------------------------\r\n");
	//系统调用函数，查看照片库中照片是否存在
	if (access("./lib/libcurl/back_up_photo.jpg", F_OK) == -1) {
    ...
    char *bufPic1 = getPicBase64FromFile("./lib/libcurl/back_up_photo.jpg");
	//系统调用函数，查看用户上传的照片是否存在
	if (access("./lib/libcurl/back_up_photo.jpg", F_OK) == -1) {
		printf("Sorry, I can't find your photo in the specified directory.\r\n");
		return FALSE;
	} else {
		printf("Successfully found your photos in the specified directory.\r\n");
	}
    char *bufPic2 = getPicBase64FromFile("./tmp/photo.jpg");
	printf("--------------------------------\r\n");
